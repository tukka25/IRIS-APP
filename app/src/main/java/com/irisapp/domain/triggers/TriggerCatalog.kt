package com.irisapp.domain.triggers
...
object TriggerCatalog {
    ...
    val all: List<TriggerInfo> = listOf(
        ...
        TriggerInfo(
            type = "share_sheet",
            label = "Share sheet",
            description = "Run when content is shared to IrisApp.",
            setupState = SetupState.NeedsSetup
        ),
        TriggerInfo(
            type = "battery",
        ...
        TriggerInfo(
            type = "alarm_stopped",
            label = "Alarm stopped",
            description = "Run when a IrisApp alarm is dismissed or stopped.",
            setupState = SetupState.Ready
        ),
        TriggerInfo(
            type = "app_opened",
        ...
        TriggerInfo(
            type = "voice",
            label = "Voice intent",
            description = "Run when the user speaks a voice command into the IrisApp mic. The spoken words are transcribed and fed to the LLM to generate the workflow to run.",
            setupState = SetupState.Ready
        ),
        TriggerInfo(
            type = "sound_event",
...
object TriggerRegistry {
    ...
    /**
     * Fires a workflow — delegates to [com.irisapp.platform.trigger.TriggerRegistry].
     * Kept here for backward compatibility with any code that references
     * domain.triggers.TriggerRegistry.fireWorkflow.
     */
    @Deprecated("Use platform.trigger.TriggerRegistry.fire() directly", ReplaceWith("com.irisapp.platform.trigger.TriggerRegistry.fire(context, workflow)"))
    fun fireWorkflow(context: Context, workflow: PlannedWorkflow) {
        com.irisapp.platform.trigger.TriggerRegistry.fire(context, workflow)
    }
}
