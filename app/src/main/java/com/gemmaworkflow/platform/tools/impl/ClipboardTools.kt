 ...
 * GemmaWorkflow is the active foreground app.
 *
...
class GetClipboardTextTool(private val context: Context) : Tool {
    ...
    override val description = "Reads text from the system clipboard. Only works when GemmaWorkflow is in the foreground (Android 10+ restriction)."
...
class SetClipboardTextTool(private val context: Context) : Tool {
    ...
    override suspend fun execute(input: Map<String, String>): ToolResult {
        ...
        return withContext(Dispatchers.Main) {
            runCatching {
                val cm = context.getSystemService(Context.CLIPBOARD_SERVICE) as ClipboardManager
                val clip = android.content.ClipData.newPlainText("GemmaWorkflow", text)
